...
@section('subhead')
    <title>Bullion Rate Fix - Midone - Tailwind HTML Admin Template</title>
@endsection

@section('subcontent')
    <h2 class="intro-y mt-10 text-lg font-medium">Bullion Rate Fix</h2>
    ...
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            <a href="{{ route('bullion-rate.create') }}">
                <x-base.button class="mr-2 shadow-md" variant="primary">
                    Add New Bullion Rate Fix
                </x-base.button>
            </a>
            <x-base.menu>
            ...
            <div class="mx-auto hidden text-slate-500 md:block">
                @isset($bullionRates)
                    Showing {{ $bullionRates->firstItem() }} to {{ $bullionRates->lastItem() }} of {{ $bullionRates->total() }} entries
                @endisset
            </div>
            ...
            <x-base.table class="-mt-2 border-separate border-spacing-y-[10px]">
                <x-base.table.thead>
                    <x-base.table.tr>
                        ...
                        <x-base.table.th class="whitespace-nowrap border-b-0"> Rate Cut On/Off </x-base.table.th>
                ...
                <x-base.table.tbody>
                    @isset($bullionRates)
                        @forelse ($bullionRates as $bullionRate)
                            <x-base.table.tr class="intro-x">
                                <x-base.table.td
                                    ...
                                    <div class="flex">
                                        <div class="whitespace-nowrap font-medium">{{ $bullionRate->id }}</div>
                                    </div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ $bullionRate->name ?? 'N/A' }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ $bullionRate->date?->format('d/m/Y') ?? 'N/A' }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ $bullionRate->rate_cut_on_off ?? 'N/A' }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    ...
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ number_format($bullionRate->amount ?? 0, 2) }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="relative w-56 border-b-0 bg-white py-0 shadow-[20px_3px_20px_#0000000b] before:absolute before:inset-y-0 before:left-0 before:my-auto before:block before:h-8 before:w-px before:bg-slate-200 first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600 before:dark:bg-darkmode-400">
                                    <div class="flex items-center justify-center">
                                        <a class="mr-3 flex items-center" href="{{ route('bullion-rate.edit', $bullionRate->id) }}">
                                            <x-base.lucide class="mr-1 h-4 w-4" icon="CheckSquare" />
                                            Edit
                                        </a>
                                        <a class="flex items-center text-danger" data-tw-toggle="modal"
                                            data-tw-target="#delete-confirmation-modal" href="#"
                                            data-delete-route="{{ route('bullion-rate.delete', $bullionRate->id) }}"
                                            data-delete-name="{{ $bullionRate->name ?? 'Bullion Rate #' . $bullionRate->id }}">
                        ...
                        @empty
                            <x-base.table.tr>
                                <x-base.table.td colspan="7" class="border-b-0 bg-white text-center shadow-[20px_3px_20px_#0000000b] dark:bg-darkmode-600">
                                    <div class="py-8 text-slate-500">No bullion rate records found.</div>
                                </x-base.table.td>
                            </x-base.table.tr>
                        @endforelse
                    @else
        ...
        @isset($bullionRates)
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
                <div class="w-full sm:mr-auto sm:w-auto">
                    {{ $bullionRates->onEachSide(1)->links() }}
                </div>
