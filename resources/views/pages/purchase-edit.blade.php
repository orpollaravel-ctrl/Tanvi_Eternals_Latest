                ...
                <form method="POST" action="{{ route('purchases.update', $purchase->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-12 gap-4">
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            ...
                            <x-base.form-input type="text" name="bill_number"
                                value="{{ old('bill_number', $purchase->bill_number) }}" placeholder="Enter bill number"
                                required />
                        </div>
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            ...
                            <x-base.form-input type="date" name="bill_date"
                                value="{{ old('bill_date', $purchase->bill_date?->format('Y-m-d')) }}" required />
                        </div>

                        <!-- Delivery Date 
                        <div class="col-span-12 sm:col-span-6">
                            <x-base.form-label>Delivery Date *</x-base.form-label>
                            <x-base.form-input type="date" name="delivery_date"
                                value="{{ old('delivery_date', $purchase->delivery_date?->format('Y-m-d')) }}" required />
                        </div>-->

                        <!-- Total Invoice Amount
                        <div class="col-span-12 sm:col-span-6">
                            <x-base.form-label>Total Invoice Amount *</x-base.form-label>   </div> -->
                            <x-base.form-input type="hidden" step="0.01" name="total_invoice_amount"
                        ...
                        <div class="col-span-12 sm:col-span-6">
                            ...
                            <x-base.form-file-input name="bill_photo" type="file" accept="image/*" />
                    ...
                    <div class="mt-8 mb-4">
                        ...
                        <p class="text-sm text-slate-500 mb-4">Select products from dropdown and fill in the details. Press
                            Enter to navigate between fields.</p>
                    </div>
                            ...
                            <tbody id="items-container" class="divide-y">
                                @foreach ($purchase->items as $index => $item)
                                    <tr class="item-row hover:bg-slate-50">
                                        <td class="px-4 py-3">
                                            <select class="product-select tom-select w-full"
                                                name="items[{{ $index }}][product_id]"
                                                data-item-index="{{ $index }}"
                                                data-placeholder="Search & select product..." required>
                                                <option value="">Select product...</option>
                                                @foreach ($products as $product)
                                                    <option value="{{ $product->id }}"
                                                        @if ($item->product_id == $product->id) selected @endif
                                                        data-product-name="{{ $product->product_name }}"
                                                        data-rate="{{ $product->minimum_rate ?? 0 }}"
                                                        data-min-qty="{{ $product->minimum_quantity ?? 0 }}"
                                                        data-barcode="{{ $product->barcode_number ?? '' }}"
                                                        data-tool-code="{{ $product->tool_code ?? '' }}">
                                                        {{ $product->product_name }}
                                                        @if ($product->barcode_number)
                                                            - {{ $product->barcode_number }}
                                                        @endif
                                                        @if ($product->tool_code)
                                                            ({{ $product->tool_code }})
                                                        @endif
                                                    </option>
                                                @endforeach
                                        ...
                                        <td class="px-4 py-3">
                                            <input type="number" step="0.01"
                                                ...
                                                placeholder="0.00" data-item-index="{{ $index }}" required />
                                        </td>
                                        ...
                                        <td class="px-4 py-3">
                                            <input type="number" step="0.01"
                                                class="amount-input w-full border rounded px-3 py-2 text-sm bg-gray-50"
                                                name="items[{{ $index }}][amount]"
                                                value="{{ old('items.' . $index . '.amount', $item->amount) }}"
                                                placeholder="0.00" data-item-index="{{ $index }}" readonly />
                                        </td>
										
										  <td class="px-4 py-3">
                                            <select name="items[{{ $index }}][gst_percentage]"
                                                class="gst-percentage-input w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                data-item-index="{{ $index }}">
                                                <option value="">Select GST %</option>
												 <option value="0" {{ old('items.' . $index . '.gst_percentage', $item->gst_percentage) == 0 ? 'selected' : '' }}>0%</option>
                                                <option value="3" {{ old('items.' . $index . '.gst_percentage', $item->gst_percentage) == 3 ? 'selected' : '' }}>3%</option>
                                                <option value="5" {{ old('items.' . $index . '.gst_percentage', $item->gst_percentage) == 5 ? 'selected' : '' }}>5%</option>
                                                <option value="12" {{ old('items.' . $index . '.gst_percentage', $item->gst_percentage) == 12 ? 'selected' : '' }}>12%</option>
                                                <option value="18" {{ old('items.' . $index . '.gst_percentage', $item->gst_percentage) == 18 ? 'selected' : '' }}>18%</option>
                                                <option value="28" {{ old('items.' . $index . '.gst_percentage', $item->gst_percentage) == 28 ? 'selected' : '' }}>28%</option>
                                            </select>
                                        </td>

                                        <td class="px-4 py-3">
                                            <input type="number" step="0.01"
                                                class="gst-value-input w-full border rounded px-3 py-2 text-sm bg-gray-50"
                                                name="items[{{ $index }}][gst_value]"
                                                value="{{ old('items.' . $index . '.gst_value', $item->gst_value) }}"
                                                placeholder="0.00" data-item-index="{{ $index }}" readonly />
                                        </td>

                                        <td class="px-4 py-3">
                                            <input type="number" step="0.01"
                                                class="final-amount-input w-full border rounded px-3 py-2 text-sm bg-gray-50"
                                                name="items[{{ $index }}][final_amount]"
                                                value="{{ old('items.' . $index . '.final_amount', $item->final_amount) }}"
                                                placeholder="0.00" data-item-index="{{ $index }}" readonly />
                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <button type="button"
                                                class="remove-item-btn text-danger hover:text-danger/70 p-1">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function initializeItemHandlers(index) {
                    ...
                    if (productSelect && !productSelect.isInitialized) {
                        ...
                        setTimeout(() => {
                            if (!productSelect.tomselect) {
                                ...
                                // ✅ Handle pre-selected product on edit
                                const selectedId = productSelect.dataset.selectedId;
                                const selectedName = productSelect.dataset.selectedName;
                                const selectedBarcode = productSelect.dataset.selectedBarcode || '';
                                const selectedToolcode = productSelect.dataset.selectedToolcode || '';

                                if (selectedId && selectedName) {
                                    const displayText =
                                        `${selectedName}${selectedBarcode ? ' - ' + selectedBarcode : ''}${selectedToolcode ? ' (' + selectedToolcode + ')' : ''}`;
                                ...
                                tomSelectInstance.on('dropdown_open', function() {
                                    if (!tomSelectInstance.loadedSearches || !tomSelectInstance
                                        .loadedSearches['']) {
                                ...
                                productSelect.addEventListener('change', function() {
                                    ...
                                    if (this.value && tomSelectInstance.options) {
                                        ...
                                        if (selectedData && selectedData.product_name) {
                                            const productNameInput = row.querySelector(
                                                '.product-name-input');
                                            if (productNameInput) {
                    ...
                    const fields = [{
                            selector: `.expiry-date-input[data-item-index="${index}"]`,
                            nextField: 'quantity-input'
                        },
                        {
                            selector: `.quantity-input[data-item-index="${index}"]`,
                            nextField: 'rate-input'
                        },
                        {
                            selector: `.rate-input[data-item-index="${index}"]`,
                            nextField: 'amount-input'
                        },
                        {
                            selector: `.amount-input[data-item-index="${index}"]`,
                            nextField: null
                        }
                    ];
