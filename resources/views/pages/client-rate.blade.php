...
@section('subhead')
    <title>Client Rate Fix - Midone - Tailwind HTML Admin Template</title>
@endsection

@section('subcontent')
    <h2 class="intro-y mt-10 text-lg font-medium">Client Rate Fix</h2>
    ...
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            <a href="{{ route('client-rate-fix.create') }}">
                <x-base.button class="mr-2 shadow-md" variant="primary">
                    Add New Client Rate Fix
                </x-base.button>
            </a>
            <x-base.menu>
            ...
            <div class="mx-auto hidden text-slate-500 md:block">
                @isset($clientRates)
                    Showing {{ $clientRates->firstItem() }} to {{ $clientRates->lastItem() }} of {{ $clientRates->total() }} entries
                @endisset
            </div>
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <x-base.table class="-mt-2 border-separate border-spacing-y-[10px]">
                ...
                <x-base.table.tbody>
                    @isset($clientRates)
                        @forelse ($clientRates as $clientRate)
                            <x-base.table.tr class="intro-x">
                                <x-base.table.td
                                    ...
                                    <div class="flex">
                                        <div class="whitespace-nowrap font-medium">{{ $clientRate->serial_no }}</div>
                                    </div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="w-40 border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="flex">
                                        <div class="whitespace-nowrap font-medium">{{ $clientRate->transaction_no }}</div>
                                    </div>
                                </x-base.table.td>
                                <x-base.table.td
                                ...
                                <x-base.table.td
                                    ...
                                    <div class="flex items-center justify-center">
                                        <a class="mr-3 flex items-center" href="{{ route('client-rate-fix.edit', $clientRate->id) }}">
                                            <x-base.lucide class="mr-1 h-4 w-4" icon="CheckSquare" />
                                            Edit
                                        </a>
                                        <a class="flex items-center text-danger" data-tw-toggle="modal"
                                            data-tw-target="#delete-confirmation-modal" href="#"
                                            data-delete-route="{{ route('client-rate-fix.delete', $clientRate->id) }}"
                        ...
                        @empty
                            <x-base.table.tr>
                                <x-base.table.td colspan="6" class="border-b-0 bg-white text-center shadow-[20px_3px_20px_#0000000b] dark:bg-darkmode-600">
                                    <div class="py-8 text-slate-500">No client rate fix records found.</div>
                                </x-base.table.td>
                            </x-base.table.tr>
                        @endforelse
                    @else
        ...
        @isset($clientRates)
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
                <div class="w-full sm:mr-auto sm:w-auto">
                    {{ $clientRates->onEachSide(1)->links() }}
                </div>
