...
@section('subhead')
    <title>Client Rate Cut Pending - Midone - Tailwind HTML Admin Template</title>
@endsection

@section('subcontent')
    <h2 class="intro-y mt-10 text-lg font-medium">Client Rate Cut Pending</h2>
    ...
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            <a href="{{ route('client-rate-cut-pending.create') }}">
                <x-base.button class="mr-2 shadow-md" variant="primary">
                    Add New Client Rate Cut Pending
                </x-base.button>
            </a>
            <x-base.menu>
            ...
            <div class="mx-auto hidden text-slate-500 md:block">
                @isset($clientRateCutPendings)
                    Showing {{ $clientRateCutPendings->firstItem() }} to {{ $clientRateCutPendings->lastItem() }} of {{ $clientRateCutPendings->total() }} entries
                @endisset
            </div>
            ...
            <x-base.table class="-mt-2 border-separate border-spacing-y-[10px]">
                <x-base.table.thead>
                    <x-base.table.tr>
                        ...
                        <x-base.table.th class="whitespace-nowrap border-b-0"> Invoice No </x-base.table.th>
                        ...
                        <x-base.table.th class="whitespace-nowrap border-b-0"> Sales Person </x-base.table.th>
                ...
                <x-base.table.tbody>
                    @isset($clientRateCutPendings)
                        @forelse ($clientRateCutPendings as $pending)
                            <x-base.table.tr class="intro-x">
                                <x-base.table.td
                                    ...
                                    <div class="flex">
                                        <div class="whitespace-nowrap font-medium">{{ $pending->id }}</div>
                                    </div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ $pending->invoice_no ?? 'N/A' }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ $pending->client_name ?? 'N/A' }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ $pending->transaction_date?->format('d/m/Y') ?? 'N/A' }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="border-b-0 bg-white shadow-[20px_3px_20px_#0000000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ $pending->sales_person ?? 'N/A' }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    ...
                                    <div class="mt-0.5 whitespace-nowrap text-xs text-slate-500">{{ number_format($pending->amount ?? 0, 2) }}</div>
                                </x-base.table.td>
                                <x-base.table.td
                                    class="relative w-56 border-b-0 bg-white py-0 shadow-[20px_3px_20px_#0000000b] before:absolute before:inset-y-0 before:left-0 before:my-auto before:block before:h-8 before:w-px before:bg-slate-200 first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600 before:dark:bg-darkmode-400">
                                    <div class="flex items-center justify-center">
                                        <a class="mr-3 flex items-center" href="{{ route('client-rate-cut-pending.edit', $pending->id) }}">
                                            <x-base.lucide class="mr-1 h-4 w-4" icon="CheckSquare" />
                                            Edit
                                        </a>
                                        <a class="flex items-center text-danger" data-tw-toggle="modal"
                                            data-tw-target="#delete-confirmation-modal" href="#"
                                            data-delete-route="{{ route('client-rate-cut-pending.delete', $pending->id) }}"
                                            data-delete-name="{{ $pending->client_name ?? 'Client Rate Cut Pending #' . $pending->id }}">
                        ...
                        @empty
                            <x-base.table.tr>
                                <x-base.table.td colspan="8" class="border-b-0 bg-white text-center shadow-[20px_3px_20px_#0000000b] dark:bg-darkmode-600">
                                    <div class="py-8 text-slate-500">No client rate cut pending records found.</div>
                                </x-base.table.td>
                            </x-base.table.tr>
                        @endforelse
                    @else
        ...
        @isset($clientRateCutPendings)
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">
                <div class="w-full sm:mr-auto sm:w-auto">
                    {{ $clientRateCutPendings->onEachSide(1)->links() }}
                </div>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                deleteButtons.forEach(function (button) {
                    button.addEventListener('click', function () {
                        ...
                        if (deleteUserName) {
                            deleteUserName.textContent = name || 'this record';
                        }
                    });
                });
            });
