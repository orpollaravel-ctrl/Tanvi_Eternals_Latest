...
@section('subhead')
    <title>Edit Bullion Purchase - Midone</title>
@endsection

@section('subcontent')
    <h2 class="intro-y mt-10 text-lg font-medium">Edit Bullion Purchase</h2>
    <div class="mt-5 grid grid-cols-8 gap-6">
        <div class="intro-y col-span-12 lg:col-span-8">
            <div class="box p-5">
                ...
                <form method="POST" action="{{ route('bullion-purchase.update', $bullionPurchase->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-4 gap-4 mt-6">
                        <div>
                            ...
                            <x-base.form-input 
                                type="text" 
                                name="serial_no" 
                                value="{{ old('serial_no', $bullionPurchase->serial_no) }}" 
                                readonly 
                                class="bg-gray-100 text-right cursor-not-allowed"/>
                        </div>
                        <div>
                            <x-base.form-label>Transaction No*</x-base.form-label>
                            <x-base.form-input type="text" name="transaction_no" value="{{ old('transaction_no',$bullionPurchase->transaction_no) }}" placeholder="" />
                        </div>
                        <div>
                            <x-base.form-label>Transaction Date*</x-base.form-label>
                            <x-base.form-input type="date" name="transaction_date" value="{{ old('transaction_date', $bullionPurchase->transaction_date?->format('Y-m-d')) }}" />
                        </div>
                        <div>
                            <x-base.form-label>Client Name*</x-base.form-label>
                            <x-base.form-input type="text" name="name" value="{{ old('name', $bullionPurchase->name) }}" placeholder="Name" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-4 mt-3">
                        <div>
                            ...
                            <x-base.form-input type="number" step="0.001" class="text-right" id="converted_weight" name="converted_weight" value="{{ old('converted_weight', $bullionPurchase->converted_weight) }}" placeholder="0.000" />
                        </div>
                        <div>
                            <x-base.form-label>Rate per Gram</x-base.form-label>
                            <x-base.form-input type="number" step="0.01" class="text-right" id="purchase_rate" name="purchase_rate" value="{{ old('purchase_rate', $bullionPurchase->purchase_rate) }}" placeholder="0.00" />
                        </div>
                        <div>
                            <x-base.form-label>Amount</x-base.form-label>
                            <x-base.form-input type="number" step="0.01" class="text-right" id="amount" name="amount" value="{{ old('amount', $bullionPurchase->amount) }}" placeholder="0.00" readonly />
                        </div>
                    </div>
